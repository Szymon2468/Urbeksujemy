...
function PolandMap({ setClickedState, placesForPolandMap }) {
  ...
  const handleMouseMove = (event) => {
    const desription = document.getElementById('description');
    const textPositionY = event.screenY - 170;
    const textPositionX = event.screenX - 20;
    desription.style.top = textPositionY + 'px';
    desription.style.left = textPositionX + 'px';
    desription.style.visibility = 'visible';
  };
  ...
  const handleMouseLeave = () => {
    const desription = document.getElementById('description');
    desription.style.visibility = 'hidden';
  };
  ...
  const handleStateName = () => {
    ...
    let name = '';
    name += stateName[0].toUpperCase();
    for (let i = 1; i < stateName.length; i++) {
      if (stateName[i] === '-') {
        name += ' ';
        i++;
        name += stateName[i].toUpperCase();
      } else {
        name += stateName[i];
      }
    }
